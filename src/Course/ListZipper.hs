...
moveLeftN' n z@(ListZipper l _ r ) = 
  case moveLeftN n z of 
   IsZ a -> Right a
   IsNotZ -> Left (if n > 0 then length l else length r )
...
moveRightN' n z@(ListZipper l _ r ) = 
  case moveRightN n z of 
   IsZ a -> Right a
   IsNotZ -> Left (if n > 0 then length r else length l )
...
instance Extend ListZipper where
  ...
  --   error "todo"
  -- Note: 
  --       The difficult part is how to generate (List b)
  --       We can't do: moveLeft on b directly, but we can get b from a!
  --       a' = moveLeft a
  --       b' = f a'
  (<<=) ::  (ListZipper a -> b) -> ListZipper a -> ListZipper b
  f <<= z = ListZipper l x r
            where l = unfoldr (\a -> case moveLeft a of 
                                      IsNotZ -> Empty
                                      IsZ a' -> Full (f a', a')) z
                  x = f z
                  r = unfoldr (\a -> case moveRight a of 
                                      IsNotZ -> Empty
                                      IsZ a' -> Full (f a', a')) z

test_unfoldr :: (ListZipper a -> b) -> ListZipper a -> List b
test_unfoldr f z = unfoldr (\a -> case moveLeft a of 
                           IsNotZ -> Empty
                           IsZ a' -> Full (f a', a')) z
